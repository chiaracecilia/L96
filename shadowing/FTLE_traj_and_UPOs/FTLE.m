%% this function calculates the FTLE given the starting point and the integration interval 

function [Lexp_cum, Lexp_ist] = FTLE(stept, X, FTLE_time, F, M)
...
% Lyapunov algorithm specification
tstart = 0;
nit = round((FTLE_time-tstart)/stept); %  Number of iterations of the exponent algorithm
time=stept*[0:nit];
...
%     end
    ...
    Y = midpoint(@lorenz96_ext_midpoint, time(ITERLYAP),dt, time(ITERLYAP+1), y, M, F); % solve the variational equation in the interval [t, tstep]
%     figure(2)
%     plot3(x(:,1), x(:,2),x(:,3), 'k') % original orbit
%     hold on
